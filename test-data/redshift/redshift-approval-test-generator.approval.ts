...
const redshiftTests = async (credentials) => {
  const redshift = new Redshift({ credentials: credentials, region: region });

  const notExistsClusterName = "this-does-not-exist";
  ...
  const deniedCreds = await credentialsForRole(roleNameWithoutAccess);
  const redshiftDenied = new Redshift({
    credentials: deniedCreds,
    region: region,
  });

  redshiftDenied
    .describeClusters({ ClusterIdentifier: existingClusterName })
    .promise()
    .catch(writeTo("redshift/describeClusters-denied.json"));
};

describe("Redshift Approval Tests", () => {
  const createEnvironment = false;
  const tearDownEnvironment = true;
  const stackName = "beady-eye-approval-redshift";
  ...
  beforeAll(async (done) => {
    ...
    if (createEnvironment) {
      const cf = new CloudFormation(this.clientParams);
      try {
        await cf
          .createStack({
            StackName: stackName,
        ...
        console.log(
          "Ignoring failure to create stack - does it already exist?"
        );
        console.log(err);
      }
    }
  ...
  afterAll(async (done) => {
    if (tearDownEnvironment) {
      const cf = new CloudFormation(this.clientParams);
      const response = await cf.deleteStack({ StackName: stackName }).promise();
    }
    done();
  }, 10 * 60 * 1000);
  ...
  xit("should generate data for the subnets", async (done) => {
    const redshift = new Redshift(this.clientParams);

    existingClusterName =
      "beady-eye-approval-redshift-redshiftcluster-gqlpmk80mi5o";
    redshift
      .describeClusters({ ClusterIdentifier: existingClusterName })
      .promise()
      .then(writeTo("redshift/describeClusters-exists.json"));
    const clusterDetails = await redshift
      .describeClusters({ ClusterIdentifier: existingClusterName })
      .promise();
    const securityGroupIds: Array<string> = clusterDetails.Clusters![0].VpcSecurityGroups!.map(
      (securityGroup: Redshift.VpcSecurityGroupMembership): string => {
    ...
    const ec2 = new EC2(this.clientParams);
    ...
    ec2
      ...
      .promise()
      .then(writeTo("redshift/ec2-describe-security-groups.json"));
    done();
  });
});

const redshiftPort = "5439";
