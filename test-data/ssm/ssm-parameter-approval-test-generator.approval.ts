...
xdescribe("SSMParameter Approval Tests", () => {
  ...
  it('should generate data for non-existent parameter', async () => {
    await ssm.getParameter({ Name: 'non-existant-parameter' }).promise().catch(writeTo('ssm/getParameter-notExists.json'));
  }, 30 * 1000)

  describe('existing parameter', () => {
    const Name = uuid();
    beforeAll(async () => {
      await ssm.putParameter({ Name, Value: 'sampleValue', Type: 'String' }).promise();
    }, 20 * 1000)
    ...
    it('should generate data for an existing parameter', async () => {
      await ssm.getParameter({ Name }).promise().then(writeTo('ssm/get-Parameter-Exists.json'))
    }, 20 * 1000)
  })
})
