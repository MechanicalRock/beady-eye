...
describe("S3.bucket#canPutObject", () => {
  ...
  it("should return true when object exists and permission granted", async (done) => {
    AWSMock.mock(
      "S3",
      "putObject",
      awsMockCallback("test-data/putObject-granted.json")
    );

    const response = await S3.bucket(bucketName).canPutObject(bucketKey);
  ...
  it("should return false when object exists and permission denied", async (done) => {
    AWSMock.mock(
      "S3",
      "putObject",
      awsMockFailureCallback("test-data/putObject-denied.json")
    );
  ...
  it("should throw an error when the bucket does not exist", async (done) => {
    AWSMock.mock(
      "S3",
      "putObject",
      awsMockFailureCallback("test-data/putObject-notExist.json")
    );
