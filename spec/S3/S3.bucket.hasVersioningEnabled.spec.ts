...
describe("S3.bucket#hasVersioningEnabled", () => {
  ...
  it("should return true if access logging is enabled", async (done) => {
    AWSMock.mock(
      "S3",
      "getBucketVersioning",
      awsMockCallback("test-data/getBucketVersioning-enabled.json")
    );

    const response = await S3.bucket(bucketName).hasVersioningEnabled();
  ...
  it("should return false if access logging is disabled", async (done) => {
    ...
    AWSMock.mock(
      "S3",
      "getBucketVersioning",
      awsMockCallback("test-data/getBucketVersioning-disabled.json")
    );
    const result = await S3.bucket(bucketName).hasVersioningEnabled();
  ...
  it("should fail if the bucket does not exist", async (done) => {
    AWSMock.mock(
      "S3",
      "getBucketVersioning",
      awsMockFailureCallback("test-data/getBucketVersioning-notExist.json")
    );
    try {
