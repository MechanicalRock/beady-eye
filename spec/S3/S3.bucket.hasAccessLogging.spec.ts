...
describe('S3.bucket#hasAccessLogging', () => {
    ...
    it('should return true if access logging is enabled', async (done) => {
        AWSMock.mock('S3', 'getBucketLogging', awsMockCallback('test-data/getBucketLogging-enabled.json'));

        let response = await S3.bucket(bucketName).hasAccessLogging()
    ...
    it('should return false if access logging is disabled', async (done) => {
        ...
        AWSMock.mock('S3', 'getBucketLogging', awsMockCallback('test-data/getBucketLogging-disabled.json'));
        let result = await S3.bucket(bucketName).hasAccessLogging()
        expect(result).toBe(false)
        done()
    })

    it('should fail if the bucket does not exist', async(done)=> {
        AWSMock.mock('S3', 'getBucketLogging', awsMockFailureCallback('test-data/getBucketLogging-notExist.json'));
        try{
