...
export class S3Bucket {
  ...
  async s3Client() {
    if (this.lazyS3Client) {
      ...
      const temporaryCreds = this.role
        ? await this.role.credentials()
        : undefined;
      this.lazyS3Client = new AwsS3({
        params: this.bucketParams,
        credentials: temporaryCreds,
      });
      return this.lazyS3Client;
    }
  }
  ...
  async shouldExist() {
    ...
    const response = await s3.headBucket(this.bucketParams).promise();
  ...
  async canListContents() {
    const s3 = await this.s3Client();
    try {
      const response = await s3.listObjects(this.bucketParams).promise();
      expect(response).not.to.be.undefined;
      return true;
    } catch (err) {
  ...
  async hasEncryptionByDefault() {
    ...
    try {
      const response = await s3
        .getBucketEncryption(this.bucketParams)
        .promise();
      expect(response).not.to.be.undefined;
      return true;
    } catch (err) {
  ...
  async hasAccessLogging() {
    const s3 = await this.s3Client();
    const response = await s3.getBucketLogging(this.bucketParams).promise();
    expect(response).not.to.be.undefined;
  ...
  async hasVersioningEnabled() {
    const s3 = await this.s3Client();
    const response = await s3.getBucketVersioning(this.bucketParams).promise();
    expect(response).not.to.be.undefined;
  ...
  async hasLifecycleRule(lifeCycleRule) {
    const s3 = await this.s3Client();
    const response = await s3
      .getBucketLifecycleConfiguration(this.bucketParams)
      .promise();
  ...
  async isPubliclyAccessible() {
    ...
    const readAuthUsers = {
      Grantee: { URI: authenticatedUsersURI, Type: "Group" },
    };

    // interrogate the bucket ACLs
    const s3 = await this.s3Client();
    const aclResponse = await s3.getBucketAcl(this.bucketParams).promise();

    const containsGrant = (toMatch) => {
      return (
        aclResponse.Grants &&
        aclResponse.Grants.find((grant) => isMatch(grant, toMatch)) != undefined
      );
    };
    ...
    try {
      const policyResponse = await s3
        .getBucketPolicy(this.bucketParams)
        .promise();
      expect(policyResponse).not.to.be.undefined;

      const anonymousAccessStatement = {
        Effect: "Allow",
        Principal: "*",
      };
      ...
      if (policyResponse.Policy) {
        ...
        const containsStatement = (toMatch) => {
          return (
            policy.Statement &&
            policy.Statement.find((statement) => isMatch(statement, toMatch)) !=
              undefined
          );
        };
  ...
  async canPutObject(key: string) {
    ...
    try {
      const response = await s3
        .putObject({ Bucket: this.name, Key: key, Body: "sample data" })
        .promise();
      return true;
    } catch (e) {
      expect(e.code).to.equal("AccessDenied");
  ...
  async canDeleteObject(key: string) {
    ...
    try {
      const response = await s3
        .deleteObject({ Bucket: this.name, Key: key })
        .promise();
      return true;
    } catch (e) {
      expect(e.code).to.equal("AccessDenied");
