...
export class DynamoDbTable {
    ...
    public hasEncryptionByDefault = async (): Promise<boolean> => {
        const dynamoDb = await this.initDynamoDb();
        const table = (await dynamoDb.describeTable({ TableName: this.tableName }).promise()).Table!;

        const encryptionStatus = (table as DynamoDB.TableDescription).SSEDescription!;
