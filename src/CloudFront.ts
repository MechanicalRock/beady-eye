...
export class CloudFrontDistribution {
  ...
  async cloudFrontClient() {
    if (this.lazyAWSCloudFrontClient) {
      ...
      const temporaryCreds = this.role
        ...
        : undefined;
      this.lazyAWSCloudFrontClient = new AwsCloudFront({
        params: this.cloudFrontParams,
  ...
  async shouldExist() {
    const cloudFrontClient = await this.cloudFrontClient();
    try {
      const response = await cloudFrontClient
        .getDistribution(this.cloudFrontParams)
        .promise();
  ...
  async isSendingLogsWithin(periodHours: number, logsBucket: S3Bucket) {
    const prefix = new CFLogsS3Object(this.distributionId).generateIdPrefix(
      new Date(),
      periodHours
    );
...
export class CFLogsS3Object {
  ...
  generateIdPrefix(keyDate: Date, periodHours: number): string {
    const earliestDate = new Date(
      keyDate.getTime() - periodHours * 60 * 60 * 1000
    );
