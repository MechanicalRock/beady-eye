...
export class VPC {
  ...
  vpcClient() {
    ...
    const client = this.lazyEc2Client || this.makeEc2Client();
    return client;
  }
  ...
  async shouldHaveS3Endpoint() {
    ...
    const queryResult: boolean =
      !!result && !!result.VpcEndpoints && result.VpcEndpoints.length == 1;
    return queryResult;
  }
  ...
  async shouldHaveRunningBastionInstance() {
    ...
    let queryResult: boolean =
      !!result && !!result.Reservations && result.Reservations.length == 1;
    if (queryResult) {
      ...
      if (res && res.Instances) {
        ...
        queryResult =
          queryResult || (instance.State && instance.State.Name == "running");
      } else {
        queryResult = false;
      }
    }
